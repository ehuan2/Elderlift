...
def register():  # the register route
    ...
    if form.validate_on_submit():  # if it's a post method and is validated
        ...
        response = send_http_request(cookies={'Cookie':request.cookies.get('user_cookies')},
            url="http://127.0.0.1:8080/v1/api/register", method="POST", body=body)
...
def register_token():  # the route to authenticate through the use of a token
    ...
    if form.validate_on_submit():  # if valid
        ...
        response = send_http_request(cookies={'Cookie':request.cookies.get('user_cookies')},
            url=f'http://127.0.0.1:8080/v1/api/register/{form.token.data}', method='GET', body={})
...
def reset_password():
    ...
    if form.validate_on_submit():
        ...
        response = send_http_request(cookies={'Cookie':request.cookies.get('user_cookies')},url = "http://127.0.0.1:8080/v1/api/reset_password", method = 'POST', body = body)
...
def reset_password_token():
    ...
    if form.validate_on_submit():
        ...
        response = send_http_request(cookies={'Cookie':request.cookies.get('user_cookies')},url = f"http://127.0.0.1:8080/v1/api/reset_password/{form.token.data}", method = 'POST', body = body)
...
def login():  # the login route
    ...
    if form.validate_on_submit():  # if the form is valid
        ...
        response = send_http_request(cookies={'Cookie':request.cookies.get('user_cookies')},
            url="http://127.0.0.1:8080/v1/api/login", method='POST', body=user_info)
...
def logout():  # the logout route
    ...
    response = send_http_request(cookies={'Cookie':request.cookies.get('user_cookies')},
        url="http://127.0.0.1:8080/v1/api/logout", method='GET')  # logs them out
...
def account():
    ...
    if request.method == 'POST':  # if it's a post request
        ...
        if form.validate_on_submit():  # if it's valid
            ...
            response = send_http_request(cookies={'Cookie':request.cookies.get('user_cookies')},
                url="http://127.0.0.1:8080/v1/api/account", method='PUT', body=body)
    ...
    response = send_http_request(cookies={'Cookie':request.cookies.get('user_cookies')},
        url="http://127.0.0.1:8080/v1/api/account", method='GET')
...
def account_delete():
    ...
    response = send_http_request(cookies={'Cookie':request.cookies.get('user_cookies')},
        url="http://127.0.0.1:8080/v1/api/account", method='DELETE')
...
def user_by_id(user_id):  # route for a specific user based on its user id
    ...
    response = json.loads(send_http_request(cookies={'Cookie':request.cookies.get('user_cookies')},
        url=f"http://127.0.0.1:8080/v1/api/user/{user_id}", method='GET').content).get("user")
...
def users_tasks():  # get the logged in user's tasks
    ...
    user = json.loads(send_http_request(cookies={'Cookie':request.cookies.get('user_cookies')},
        url="http://127.0.0.1:8080/v1/api/account", method='GET').content).get("user")

    tasks = json.loads(send_http_request(cookies={'Cookie':request.cookies.get('user_cookies')},
        url=f"http://127.0.0.1:8080/v1/api/user/{user.get('id')}/tasks?page={page}", method='GET').content)
