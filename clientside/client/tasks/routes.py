...
def task_by_id(task_id):
    ...
    response = send_http_request(
        url=f"http://127.0.0.1:8080/v1/api/task/{task_id}", method='GET', cookies={'Cookie':request.cookies.get('user_cookies')})
    ...
    if is_logged_in(cookies={'Cookie':request.cookies.get('user_cookies')}):
        ...
        user = json.loads(send_http_request(
            url="http://127.0.0.1:8080/v1/api/account", method='GET', cookies={'Cookie':request.cookies.get('user_cookies')}).content).get("user")
    ...
    if is_logged_in(cookies={'Cookie':request.cookies.get('user_cookies')}):
        ...
        if user.get("user_role") == 'Elderly':
            ...
            if form.validate_on_submit():
                ...
                response = send_http_request(
                    url="http://127.0.0.1:8080/v1/api/elderly/new_task", method='POST', body = body, cookies={'Cookie':request.cookies.get('user_cookies')}).content.decode("utf-8")
...
def elderly_delete_task(task_id):
    ...
    if is_logged_in(cookies={'Cookie':request.cookies.get('user_cookies')}):
        ...
        url=f"http://127.0.0.1:8080/v1/api/task/{task_id}", method='GET', cookies={'Cookie':request.cookies.get('user_cookies')}).content)


        # if the elderly of the task is the current user, allow deletions
        if user.get("id") == task.get("elderly_id"):
            response = send_http_request(url = f"http://127.0.0.1:8080/v1/api/elderly/{task_id}", method = "DELETE", cookies={'Cookie':request.cookies.get('user_cookies')})
...
def elderly_update_task(task_id): # this is to update a task
    ...
    if form.validate_on_submit(): # if the form is valid, then it sends a put request
        ...
        response = send_http_request(url = f"http://127.0.0.1:8080/v1/api/elderly/{task_id}", method = "PUT", body = body, cookies={'Cookie':request.cookies.get('user_cookies')})
        flash(f"{response.content.decode('utf-8')}", "success")
...
def task_doer_add_task(task_id):
    ...
    response = send_http_request(url = f"http://127.0.0.1:8080/v1/api/task_doer/{task_id}", method = "POST", cookies={'Cookie':request.cookies.get('user_cookies')})
...
def task_doer_remove_task(task_id):
    ...
    # sends a request and then flashes a success and redirects to the task
    response = send_http_request(url = f"http://127.0.0.1:8080/v1/api/task_doer/{task_id}", method = "DELETE", cookies={'Cookie':request.cookies.get('user_cookies')})
    flash(response.content.decode("utf-8"), "success")
    return redirect(url_for("tasks.task_by_id", task_id = task_id))